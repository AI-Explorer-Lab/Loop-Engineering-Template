...
"${LOOP_ENGINEERING_PYTHON}" -c 'import dynaconf, fastapi, mcp, openai_codex, pydantic, uvicorn' \
  >/dev/null 2>&1 \
  || fail "loop-engineering 环境缺少依赖，请先安装 requirements.txt 和 backend/requirements.txt。"

[[ -x "${ORCHESTRATOR_DIR}/node_modules/.bin/codex" ]] \
  || fail "Codex runtime 未安装，请在仓库根目录运行 npm ci。"
[[ -x "${FRONTEND_DIR}/node_modules/.bin/vite" ]] \
  || fail "编排器前端依赖未安装，请运行 npm ci --prefix frontend。"
...
"${LOOP_ENGINEERING_PYTHON}" -m uvicorn backend.main:app \
  --host 127.0.0.1 \
  --port "${BACKEND_PORT}" &
BACKEND_PID=$!
